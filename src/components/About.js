...
export default function About() {
  ...
  return (
    <div id="about" className="sm:min-h-[600px] md:min-h-screen bg-[var(--primary)] grid sm:grid-cols-[1fr_1fr] items-center px-20 relative">
        <div>
            ...
            <button className="bg-[var(--secondary)] hover:bg-[var(--hover)] rounded p-btn m-btn z-2 relative">Connect Now</button>
        </div>

        <div className={`flex justify-center transform md:translate-x-4 max-sm:h-48 sm:-translate-y-12 gap-x-6 ${screenSize == 'md' ? 'scale-80' : screenSize == 'sm' ? 'scale-70' : screenSize == 'xs' ? 'scale-70 translate-x-12 -translate-y-20 opacity-40' : ''}`}>
            <motion.div 
                initial={{ rotate: 0 }} 
